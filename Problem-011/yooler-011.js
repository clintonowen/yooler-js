...
function getLargestGridProduct () {
  ...
  for (let i = 0; i < grid.length - 3; i++) {
    if (i % 18 !== 0 && i % 19 !== 0 && i % 20 !== 0) {
      ...
      let product = getProduct(i, 1, grid);
      if (product > maxProduct) {
        maxProduct = product;
      }
      // Check Diagonal-Right product
      product = getProduct(i, 21, grid);
      if (product > maxProduct) {
        maxProduct = product;
      }
      if (i > 60) {
        // Check Diagonal-Left product
        product = getProduct(i, 19, grid);
        if (product > maxProduct) {
          maxProduct = product;
        }
      }
    ...
    if (i <= 340) {
      ...
      let product = getProduct(i, 20, grid);
      if (product > maxProduct) {
        maxProduct = product;
      }
    }
  ...
  return `The greatest product of four adjacent numbers is ${maxProduct}.`;
}
